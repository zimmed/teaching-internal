...
var getStats = function (assignment) {
    ...
    return (json[assignment]) ? json.basePath : '';
};
...
var addSubmission = function (astats, user, assign, path, date) {
    ...
    return new Promise(function (resolve, reject) {
        var sub, dir;
        console.log('Inside submission promise: ' + user + ' - ' + path);
        fs.stat(path, function (err, stats) {
            ...
            console.log('\tObtained stats: ' + JSON.stringify(stats));
            Submission.count({user: user, assign: assign}, function (err, count) {
                ...
                var output = astats + '/' + user + '/' + assign + '/' + count + '/';
                if (dir) {
                    console.log('\tExecuting acp-r...');
                    exec("sh /bin/teachingscripts/acp-r.sh '" +
                        sub.dirPath + "/*' '" + output + "'", function (err) {
                        if (err) {
                    ...
                    console.log('\tExecuting acp...');
                    exec("sh /bin/teachingscripts/acp.sh '" +
                         sub.fullPath + "' '" + output + "'", function (err) {
                        if (err) {
                            console.log('cproc err');
                            console.log(err);
                            reject(err);
